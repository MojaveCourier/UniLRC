...
void
gf_gen_cauchy_matrix(unsigned char **a, int m, int k)
{
    for (int i = 0; i < m - k; i++)
        for (int j = 0; j < k; j++){
                a[i][j] = gf_inv((i + k) ^ j);
...
void
gf_gen_local_vector(unsigned char *a, int k, int p)
{
    int i;
...
void 
encode(int k, int r, int z, int data_num, unsigned char **data_ptrs, 
